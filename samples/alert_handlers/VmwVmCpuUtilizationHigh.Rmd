...
## - uncomment and use Knit button in RStudio
#inputArgs <- c("CENTRAL","VmwVmCpuUtilizationHigh","vSphereE~VMIRIS1061",1035,1,"High Warning Limit exceeded current value: 69.26 limit: 50.0")
...
alert_index
host <- getCacheHistory(rtvDataserver,cache="VmwVirtualMachines",rtvquery=rtvquery,
                        fval=alert_index[2],fcol="name",
                        col="time_stamp;cpu.usage.average;mem.usage.average",tr=600)
```

```{r, echo=FALSE}
# plot labels
# for nice plot scale, set time bounds to upper and lower one-minute tics
xstart <- floor(start(host)[2]/frequency(host)*86400/60)*60
xend <- ceiling(end(host)[2]/frequency(host)*86400/60)*60
...
ticks <- seq(xstart, xend, by=60)/86400 + start(host)[1]
xlabels <- seq((xstart/60),((xend/60)),by=1) %% 60
axis(1, at=ticks, labels=xlabels)
yleg <- (host[1,1]+host[1,2])/2 + 5
legend(ticks[1],yleg,legend=c("%CPU","%Memory"), lty=line.types, col=color.scale)
```
